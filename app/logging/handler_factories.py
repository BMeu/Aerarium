...
def create_file_handler(level: int, formatter: Formatter, file: str, max_file_size: int, max_num_of_files: int)\
        -> RotatingFileHandler:
    """
        Initialize a rotating file handler inserting all events into the specified file.

        The path of ``file`` must exist!

        :param level: The minimum logging level, e.g. ``logging.INFO`` or ``logging.ERROR``.
        :param formatter: The logging formatter applied to the each log record.
...
def create_mail_handler(level: int, formatter: Formatter, sender: str, recipients: List[str], subject: str, server: str,
        ...
        -> SecureSMTPHandler:
    """
        Initialize a logging handler sending all events via mail.
...
def create_stream_handler(level: int, formatter: Formatter) -> StreamHandler:
    """
        Initialize a logging handler printing all events to STDOUT.
